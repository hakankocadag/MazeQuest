...
public class MazeGenerator {
    ...
    private void carvePassage(Cell current, Cell next) {
        ...
        if (rDiff == 1) { current.wallN = false; next.wallS = false; }
        else if (rDiff == -1) { current.wallS = false; next.wallN = false; }
        else if (cDiff == 1) { current.wallW = false; next.wallE = false; }
        else if (cDiff == -1) { current.wallE = false; next.wallW = false; }
    }
}
